...
def render_report(concept_text, model, endpoint, results, overall):
    lines = []
    lines.append("# Synthetic Survey Report\n")
    lines.append("**Concept/copy tested:**\n")
    lines.append(f"> {concept_text}\n")
    lines.append(f"**Model:** `{model}` via `{endpoint}`  ")
...
def main():
    ...
    print(f"Parsed {len(segments)} segment(s) from {personas_path.name}. Model: {args.model} @ {args.endpoint}\n")
    ...
    report = render_report(concept_text, args.model, args.endpoint, results, overall)
